...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        """
        # NOTICE: You can't damage the bees in the beehive
        i = 0
        place = self.place
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= amount:
            # must be ~.bees[:] instead of ~.bees, because maybe you will
            # delete some bees, after that, self.bees will be changed, and 
            # the process will be interrupted, you can see the folder named 
            # issue in this Repo.
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        selected_bee = rANTdom_else_none(bee)
        selected_bee.reduce_armor(selected_bee.armor)
...
class WallAnt(Ant):
    ...
    # def action可以不写,会继承Insect的,(Ant没有)
    def action(self, gamestate):
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        def double(place):
            while place.exit:
                place = place.exit
                if place.ant:
                    if place.ant.double_damaged == False:
                        place.ant.damage *= 2
        ...
        if self.real_queen == True:
            ...
            double(self.place)
        # The Fake Queen, you just need to reduce it's armor 
        # before any actions 
        else:
    ...
    # This method will be called by not only (class Bee -> def sting -> reduce_armor)
    # but also line 402
    def reduce_armor(self, amount):
    ...
    def remove_from(self, place):
        if (isinstance(self, QueenAnt) and not self.real_queen) or not isinstance(self, QueenAnt):
