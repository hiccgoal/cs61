...
from email.errors import FirstHeaderLineIsContinuationDefect  # Python 2 compatibility
...
def eval_all(expressions, env):
    ...
    while cur_expressions is not nil:
        if cur_expressions.rest == nil:
            return scheme_eval(cur_expressions.first, env)
        scheme_eval(cur_expressions.first, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        "*** YOUR CODE HERE ***"
        a = Frame(self)
        # Using the same dict causes error, e.g.,
        # a.bindings = self.bindings
        # Solving it with a built-in function of dict, which named copy()
        a.bindings = self.bindings.copy()
...
def do_and_form(expressions, env):
    ...
    cur_expressions = expressions
    while cur_expressions is not nil:
        value = scheme_eval(cur_expressions.first,env)
        if not is_true_primitive(value):
            return False
        if cur_expressions.rest is nil:
            return value
        cur_expressions = cur_expressions.rest
    # when expressions is nil, return True directly
    return True
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while cur_expressions is not nil:
        ...
        if is_true_primitive(value):
            return value
        cur_expressions = cur_expressions.rest
    # when expressions is nil, return False directly
    return False
    # END PROBLEM 12
