...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    #     averaged_rool_dice = make_averaged(roll_dice,trials_count=trials_count)
    #     mid = averaged_rool_dice(num_of_dice,dice)
    #     if mid>max:
    #         max = mid
    #         finest_num_rolls = num_of_dice
    #     num_of_dice += 1
    # return finest_num_rolls

    num_try = 1
    max = 0
    while num_try<=10:
        averaged_dice = make_averaged(roll_dice, 1000)
        mid=averaged_dice(num_try,dice)
        if mid > max:
            max = mid
            best_try = num_try
        num_try+=1
